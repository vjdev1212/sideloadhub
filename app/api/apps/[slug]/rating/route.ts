...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!email) return NextResponse.json({ error: "Sign in with Google to rate apps." }, { status: 401 });
  ...
  const userKey = createHash("sha256").update(`sideloadhub:${email}`).digest("hex");
