...
export default function SubmitPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setMessage("Repository submitted. It is now pending approval."); setGithubUrl("");
  ...
  return <main className="mx-auto min-h-screen max-w-3xl px-5 py-16 lg:px-8">
    ...
    <div className="mt-12"><p className="text-sm font-medium text-blue-600">Developer submission</p><h1 className="mt-2 text-5xl font-semibold tracking-tight">Add a GitHub repository.</h1><p className="mt-5 text-lg text-gray-600 dark:text-gray-300">We’ll inspect its public releases and IPA assets. Approved repositories become part of the catalog and central source.</p></div>
    <form onSubmit={submit} className="mt-10 rounded-3xl border border-black/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[.04] sm:p-8">
      ...
      <button disabled={busy} className="mt-4 rounded-full bg-black px-5 py-3 text-sm font-semibold text-white disabled:opacity-50 dark:bg-white dark:text-black">{busy ? "Checking…" : "Submit repository"}</button>
