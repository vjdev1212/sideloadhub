...
export default async function Home() {
  ...
  const viewAllLink = "rounded-full border border-black/10 px-4 py-2 text-sm font-semibold text-gray-700 transition hover:border-black/20 hover:bg-black hover:text-white dark:border-white/10 dark:text-gray-200 dark:hover:border-white/20 dark:hover:bg-white dark:hover:text-black";

  return (
    <main className="min-h-screen">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
        <Link href="/" className="text-xl font-semibold tracking-tight">SideloadHub</Link>
        <Link href="/submit" className="rounded-full bg-black px-4 py-2 text-sm font-medium text-white transition hover:-translate-y-0.5 hover:shadow-lg dark:bg-white dark:text-black">Add Repository</Link>
      </nav>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-16 pt-14 lg:px-8 lg:pb-20 lg:pt-24">
        <div className="max-w-4xl">
          ...
          <div className="mt-9"><Link href="/submit" className="inline-flex rounded-full bg-black px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-black/10 transition hover:-translate-y-0.5 hover:shadow-xl dark:bg-white dark:text-black">Add Repository</Link></div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-5 pb-20 lg:px-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div><p className="text-sm font-medium text-blue-600">Fresh from GitHub</p><h2 className="mt-1 text-3xl font-semibold tracking-tight">Recently Added</h2><p className="mt-2 text-sm text-gray-500">The 10 newest repositories in the catalog.</p></div>
          <Link href="/apps" className={viewAllLink}>View All</Link>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20 lg:px-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div><p className="text-sm font-medium text-amber-500">Community picks</p><h2 className="mt-1 text-3xl font-semibold tracking-tight">Top Starred Apps</h2><p className="mt-2 text-sm text-gray-500">Apps with the most community ratings.</p></div>
          <Link href="/apps?sort=rating" className={viewAllLink}>View All</Link>
        </div>
      ...
      <footer className="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-10 text-sm text-gray-500 sm:flex-row sm:items-center sm:justify-between lg:px-8"><span>© {new Date().getFullYear()} SideloadHub</span><Link href="/submit" className="transition hover:text-black dark:hover:text-white">Add Repository</Link></footer>
    </main>
  );
}
